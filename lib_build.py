...
try:
    ...
    if os.path.isdir(KERNEL_PATH):
        ...
        if not freertos_tag:
            sys.stderr.write(
                "Could not get remote tags - the local kernel (%s) may not be the most up-to-date\n"
                % tags[0]
            )
    else:
        sys.stderr.write(
            "Could not get remote tags - check you can access %s:\n" % KERNEL_URL
        )
        env.Exit(1)
...
if not freertos_tag:
    ...
    if not tag_name:
        sys.stderr.write(
            "PlatformIO-FreeRTOS: Tag '%s' could not be found in local/remote repository!\n"
            % freertos_tag
        )
        env.Exit(1)
...
if not (os.path.isdir(KERNEL_PATH)):
    try:
        ...
        sys.stderr.write(
            "Could not clone Kernel - check you can access %s:\n" % KERNEL_URL
        )
        env.Exit(1)
else:
...
if len(port) == 0:
    sys.stderr.write(
        "Error: No device defined for FreeRTOS! Please provide platform manually - options below:\n"
    )
